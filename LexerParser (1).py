...
def p_addDir(p):
    ...
    addDir :
    '''
    global directorio_fun, current_func, varTable, current_type, countCuad, memoriaVir, memoriaTemp, tempcount, ptype,vFinal,vControl
...
def p_genFor(p):
    ...
    genFor : 
    '''
    global current_type, memoriaVir, pOper, countCuad,vControl
    # Revisar que el valor sea entero
    if current_type == "int":
        # Obtenemos el valor de su respectiva pila
        #arreglar for -- quitar lo de v control ahora se controla con las variables 
        exp = pilaO.pop()
        pOper.append(p[-1])
        vc = pilaO[-1]
...
def p_compFor(p):
    ...
    compFor : 
    '''    
    global pilaO, current_type, memoriaVir, current_func, directorio_fun, pjumps, countCuad, cuad, cont,vFinal,vControl
    # Obtenemos el valor de su respectiva pila
    exp = pilaO.pop()
    # Revisar que el valor sea entero
    if current_type == "int":
        ...
        #arreglar for---agregar una pila de vfianal para saber en cual esta
        ...
        vFinal += 1   
        #arreglar for---quitar v control ya no se necesita
        # Generamos temporal
...
def p_actFor(p):
    ...
    actFor : 
    '''
    global directorio_fun, current_func, memoriaVir, cuad, countCuad, pjumps, pilaO,vControl
    # Obtenemos el vControl
    resultado = memoriaVir["intTemp"]
    memoriaVir["intTemp"] += 1
    ...
    for node in directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"]:
        ...
        if lenNodes != dim:
            directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"][node].append(m)
             ...
             #arreglar arreglos --quitar donde se pone la memora
            k = offset
...
def p_crearCuadA(p):
    ...
    crearCuadA : 
    '''
    global dimNode, cuad, tempcount, pilaO, pilaDim, memoriaVir, directorio_fun, current_name,arraux,ptype
    # Obtenemos el numero que se le ingreso al arreglo
   #arreglado para arreglos --- poner dirrecion de memoria a los limites
    memoriaVir, ptype, current_type, pilaO = generaMemoriaConst("intConst", memoriaVir, "int", ptype, pilaO)
    ...
    cuad.append(cuadruplos("VERIFY",resultado ,diminf, dimsup))
    ...
    if (pilaDim[-1] + 1) in directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"]:
        ...
        tempcount += 1
        #arreglar areglos ---poner memoria a la m
        memoriaVir, ptype, current_type, pilaO = generaMemoriaConst("intConst", memoriaVir, "int", ptype, pilaO)
        constTable[pilaO[-1]] = {"Type" : current_type, pilaO[-1] :int(m)}
        ptype.pop()
        cuad.append(cuadruplos('*',aux,pilaO.pop(),result))
        #arreglar arreglos -- poner la memoria en la pila
...
def p_crearCuadB(p):
    ...
    crearCuadB : 
    '''
    global pilaO, cuad, k,ptype,current_type,memoriaVir
    ...
    # arreglar arreglos poner la definicon de memoria aqui
    memoriaVir, ptype, current_type, pilaO = generaMemoriaConst("intConst", memoriaVir, "int", ptype, pilaO)
    constTable[pilaO[-1]] = {"Type" : current_type, pilaO[-1] :int(k)}
    ptype.pop()
    cuad.append(cuadruplos('+',  aux, pilaO.pop(),  temp1))
